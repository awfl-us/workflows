...
object Tools {
  ...
  // case class Runner(
  //   defn: Tool,
  //   run: (BaseValue[ToolCall], BaseValue[Double], Value[String]) => (List[Step[?, ?]], BaseValue[String])
  // )

  // Shared param shapes
  private val updateFileParams = ToolDefProperty(
    `type` = "object",
    properties = Map(
      "filepath" -> ToolDefProperty("string"),
      "content"  -> ToolDefProperty("string")
    ),
    required = OptList(List("filepath", "content").cel)
  )

  private val readFileParams = ToolDefProperty(
    `type` = "object",
    properties = Map(
      "filepath" -> ToolDefProperty("string")
    ),
    required = OptList(List("filepath").cel)
  )

  private val runCommandParams = ToolDefProperty(
    `type` = "object",
    properties = Map(
      "command" -> ToolDefProperty("string")
    ),
    required = OptList(List("command").cel)
  )

  // Public query-only params for simple "{ query }" tools
  val queryOnlyParams = ToolDefProperty(
    `type` = "object",
    properties = Map(
      "query" -> ToolDefProperty("string")
    ),
    required = OptList(List("query").cel)
  )
  ...
  val queryWithTaskParams = ToolDefProperty(
    `type` = "object",
    properties = Map(
      "query" -> ToolDefProperty("string"),
  ...
  //   def enqueueRun(opName: String): (BaseValue[ToolCall], BaseValue[Double], Value[String]) => (List[Step[?, ?]], BaseValue[String]) =
  ...
  //   val runFn: (BaseValue[ToolCall], BaseValue[Double], Value[String]) => (List[Step[?, ?]], BaseValue[String]) = { (toolCall, _, _) =>
