...
trait ToolWorkflow extends us.awfl.core.Workflow {
  ...
  // def apply(name: String, sessionId: Value[String], query: BaseValue[String], model: Value[String], fund: BaseValue[Double], background: Boolean): Call[RunWorkflowArgs[EventHandler.Input], ChatToolResponse] = {
