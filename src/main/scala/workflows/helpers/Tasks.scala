...
import utils.{get, PostResult}
import workflows.EventHandler
import workflows.tools.Tools
import services.Llm.{Tool, ToolFunctionDef, ToolDefParams, ToolDefProperty}
import utils.Env

object Tasks extends core.Workflow {
  ...
  case class Input(env: BaseValue[utils.Env] = utils.ENV)
  ...
  def createTask(name: String, title: BaseValue[String], description: BaseValue[String], status: BaseValue[String]) = {
    utils.post[CreateTaskBody, TaskEnvelope](
      name,
      "tasks",
  ...
  def updateTask(name: String, id: BaseValue[String], title: BaseValue[String], description: BaseValue[String], status: BaseValue[String]) = {
    utils.patchV[UpdateTaskBody, TaskEnvelope](
      name,
