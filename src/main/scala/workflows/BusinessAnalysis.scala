...
object BusinessAnalysis extends core.Workflow {
  ...
  val zipCompetitorReviews = utils.zip[Competitor, Reviews, CompetitorAndReviews](
  ...
  def status(s: String, msg: Cel) = services.Firebase.update(s"Status_${s}", str("businesses.report"), input.placeId, obj(Map("status" -> s, "statusMsg" -> str(msg))))
  ...
  val analyzeReport = services.Llm.chatJson[AnalysisResponse]("analyze_business", buildPrompt.resultValue)
  ...
  val cacheReport = utils.Cache("cacheReport", str("businesses.report"), input.placeId, cacheTtl, Block("cacheReportBlock", List[Step[_, _]](
    status("started", "Generating keywords..."),
    generateKeywords,
    // services.Firebase.create("createRecord", "businesses", input.placeId, generateKeywords.resultValue),
