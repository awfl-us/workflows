...
- Preload mechanics
  - Use workflows.helpers.Context.preloadFile(absPath) to read files at workflow start and compose them into the system prompt.
  - Use workflows.helpers.Context.preloadCommand(name, command) to run a command and include stdout in the system prompt (e.g., list YAML gens or show current time).
