 ...
 */
trait All[In, Out](using spec: Spec[Out], yoj: Yoj[In], ista: Ista[Out], prompt: Convo.Prompt) extends core.Workflow {
  import StriderObj.*
...
trait Latest[In, Out](val nameStr: String, windowSeconds: Int = utils.Segments.DefaultWindowSeconds, overlapSeconds: Int = utils.Segments.DefaultOverlapSeconds)(using spec: Spec[Out],
                                       yoj:      Yoj[In],
                                       ista:     Ista[Out],
                                       prompt:   Convo.Prompt)
    extends All[In, Out] {
